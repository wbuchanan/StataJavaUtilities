...
public class DataSetDoubleArrays implements StataData {
	 ...
	 */
	public DataSetDoubleArrays(Meta metaobject, Number missingValue) {
	...
	public void setData() {
		...
		Double[][] obs = new Double[osize][this.metaob.getVarindex().size()];
		...
		for (Integer i = 0; i < this.metaob.getObs13().size(); i++) {
			...
			for (Integer j = 0; j < this.metaob.getVarindex().size(); j++) {
				...
				if (Data.isValueMissing(Data.getNum(metaob.getVarindex(j),  i))) {
					...
					// If value is missing, set value to NaN
					obs[i][j] =  Double.MAX_VALUE;

				} else {

					// Convert numeric variables to string
					obs[i][j] = Data.getNum(metaob.getVarindex(j), i);
		...
		for (Integer i = 0; i < this.metaob.getObs13().size(); i++) {
			...
			for (Integer j = 0; j < this.metaob.getVarindex().size(); j++) {
				...
				if (Data.isValueMissing(Data.getNum(metaob.getVarindex(j),  i))) {
					...
					// If value is missing, set value to NaN
					obs[i][j] =  missingValue.doubleValue();
